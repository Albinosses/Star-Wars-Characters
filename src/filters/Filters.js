...
import { changeName, clearFilters} from '../state/filters/filtersSlice'
    ...
    return (
        <div style={outerSpaceStyle}>
            <Masonry columns={3} spacing={2}>
                <MovieSelector key={1}/>
                <TextField key={2} onChange={handleSearchNameChanged} value={searchName} label="Character name" variant="outlined" />
                <GenderSelector key={3} />
                <MassFilter key={4} />
                <Button key={5} onClick={handleClear} variant='contained'>Clear</Button>
            </Masonry>
        </div>
    );
}
